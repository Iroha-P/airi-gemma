...
const DOCS_SECTION_PATTERN = /\/(en|zh-Hans)\/docs\/([^/]+)\//
...
export function usePrevNext() {
  ...
  return computed(() => {
    ...
    let candidates = uniqBy(links, link => link.link.replace(URL_QUERY_OR_HASH_PATTERN, ''))
    ...
    if (sectionPrefix) {
      ...
      const isSectionRoot = currentFullUrl.replace(URL_QUERY_OR_HASH_PATTERN, '') === sectionBase
      if (isSectionRoot) {
        return {
          prev: undefined,
          next: undefined,
      ...
      const filtered = candidates
        .filter(l => l.link.replace(URL_QUERY_OR_HASH_PATTERN, '').startsWith(sectionBase))
        .filter(l => l.link.replace(URL_QUERY_OR_HASH_PATTERN, '') !== sectionBase)
...
function getDocsSectionPrefix(fullUrl: string): string | undefined {
  const m = fullUrl.match(DOCS_SECTION_PATTERN)
