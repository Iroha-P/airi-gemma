...
const AT_ASSETS_PATTERN = /^@assets\(('\S+')|("\S+")|(\S+)\)$/
const LEADING_PAREN_PATTERN = /^\(/
const TRAILING_PAREN_PATTERN = /\)$/
const LEADING_SINGLE_QUOTE_PATTERN = /^'/
const TRAILING_SINGLE_QUOTE_PATTERN = /'$/
const LEADING_DOUBLE_QUOTE_PATTERN = /^"/
const TRAILING_DOUBLE_QUOTE_PATTERN = /"$/
const SVG_EXT_PATTERN = /^svg$/i

function fromAtAssets(url: string): string {
  if (AT_ASSETS_PATTERN.test(url)) {
    const res = url
      .trim()
      .replace(AT_ASSETS_PATTERN, '$1')
      .replace(LEADING_PAREN_PATTERN, '')
      .replace(TRAILING_PAREN_PATTERN, '')
      .replace(LEADING_SINGLE_QUOTE_PATTERN, '')
      .replace(TRAILING_SINGLE_QUOTE_PATTERN, '')
      .replace(LEADING_DOUBLE_QUOTE_PATTERN, '')
      .replace(TRAILING_DOUBLE_QUOTE_PATTERN, '')
...
function recursivelyFindAtAssets(propertyMaybeObjectOrScalar: unknown, fn: (value: string) => string | undefined) {
  if (typeof propertyMaybeObjectOrScalar === 'string') {
    if (AT_ASSETS_PATTERN.test(propertyMaybeObjectOrScalar)) {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        res.writeHead(200, {
          'Content-Type': SVG_EXT_PATTERN.test(ext) ? 'image/svg+xml' : `image/${ext}`,
