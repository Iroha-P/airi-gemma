...
const JS_FENCE_LANGUAGES = new Set(['', 'javascript', 'js', 'ts', 'typescript'])

function extractFencedCode(input: string): string | undefined {
  ...
  const language = input.slice(3, newlineIndex).trim().toLowerCase()
  if (!JS_FENCE_LANGUAGES.has(language))
    return undefined

  return input.slice(newlineIndex + 1, -3).trim()
}
...
function __plannerExtractEventSummary(content) {
  const markerIndex = content.indexOf('[EVENT]')
  if (markerIndex === -1)
    return undefined

  const line = content
    .slice(markerIndex + '[EVENT]'.length)
    .trimStart()
    .split('\n')[0]
    ?.trim()

  return line && line.includes(':') ? line : undefined
}
  ...
  return {
    ...
    playerChats: (n = 5) => {
      ...
      for (let i = conversationHistory.length - 1; i >= 0 && chats.length < n; i--) {
        ...
        const summary = __plannerExtractEventSummary(msg.content)
        if (summary && !summary.startsWith('Perception Signal:'))
