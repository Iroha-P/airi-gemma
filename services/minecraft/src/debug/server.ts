...
const ISO_TIME_COLON_PATTERN = /:/g
const DIRECTORY_TRAVERSAL_PREFIX_PATTERN = /^(\.\.[/\\])+/
...
export class DebugServer {
  ...
  private initLogFile(): void {
    ...
    const filename = `session-${new Date().toISOString().replace(ISO_TIME_COLON_PATTERN, '-')}.jsonl`
  ...
  private handleHttpRequest(req: IncomingMessage, res: http.ServerResponse): void {
    ...
    filePath = path.normalize(filePath).replace(DIRECTORY_TRAVERSAL_PREFIX_PATTERN, '')
