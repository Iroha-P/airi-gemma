...
const HTML_DOUBLE_QUOTE_PATTERN = /"/g
const HTML_SINGLE_QUOTE_PATTERN = /'/g
...
function escapeHtml(text) {
  ...
  return String(text)
    ...
    .replace(HTML_DOUBLE_QUOTE_PATTERN, '&quot;')
    .replace(HTML_SINGLE_QUOTE_PATTERN, '&#39;')
}
...
function parseSectionTag(text) {
  ...
  const tag = text.slice(1, closeIndex)
  if (!EVENT_TAGS.has(tag))
    return undefined

  return { tag, textStart: closeIndex + 1 }
}
...
function isWordToken(text) {
  ...
  for (const char of text) {
    ...
    if (!isDigit && !isUpper && !isLower && char !== '_')
      return false
  }
...
function parseChatSummary(text) {
  const prefix = 'Chat from '
  if (!text.startsWith(prefix) || !text.endsWith('"'))
    return undefined

  const delimiter = ': "'
  const delimiterIndex = text.indexOf(delimiter, prefix.length)
  if (delimiterIndex === -1)
    return undefined

  const speaker = text.slice(prefix.length, delimiterIndex)
  if (!isWordToken(speaker))
    return undefined

  return {
    speaker,
    message: text.slice(delimiterIndex + delimiter.length, -1),
  }
}
...
function parseFeedbackSummary(text) {
  ...
  if (rest.startsWith('Success')) {
    status = 'Success'
    rest = rest.slice('Success'.length)
  }
...
function parseUserMessage(content) {
  ...
  const blocks = content.split(DOUBLE_NEWLINE_PATTERN)
  for (const block of blocks) {
    const trimmed = block.trim()
    if (!trimmed)
      continue
    const parsedTag = parseSectionTag(trimmed)
    if (parsedTag) {
      sections.push({ tag: parsedTag.tag, text: trimmed.slice(parsedTag.textStart).trimStart() })
    }
    else {
...
class ConversationPanel {
  ...
  summarizeEvent(section) {
    ...
    const chatSummary = parseChatSummary(text)
    if (chatSummary)
      return `Chat from ${chatSummary.speaker}: "${chatSummary.message.slice(0, 60)}${chatSummary.message.length > 60 ? '...' : ''}"`
    ...
    if (section.tag === 'FEEDBACK') {
      const feedbackSummary = parseFeedbackSummary(text)
      if (feedbackSummary) {
        const fbMatch = [undefined, feedbackSummary.name, feedbackSummary.status, feedbackSummary.detail]
        const detail = feedbackSummary.detail.slice(0, 50)
        return `${fbMatch[1]}: ${fbMatch[2]}${detail ? ` — ${detail}${fbMatch[3].length > 50 ? '...' : ''}` : ''}`
      }
    }

    // Fallback: truncate
    const flat = text.replace(NEWLINE_GLOBAL_PATTERN, ' ').slice(0, 70)
  ...
  renderUserSection(section, turnNum) {
    ...
    const preview = section.text.slice(0, 60).replace(NEWLINE_GLOBAL_PATTERN, ' ')
  ...
  renderParsedAssistantMessage(msg, turnNum) {
    ...
    if (reasoning) {
      ...
      const preview = reasoning.slice(0, 80).replace(NEWLINE_GLOBAL_PATTERN, ' ')
