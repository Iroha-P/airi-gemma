...
const TWITTER_PROFILE_URL_PATTERN = /twitter\.com\/([^/]+)/
const FOLLOWING_SUFFIX_PATTERN = /Following.*/
const FOLLOWERS_SUFFIX_PATTERN = /Followers.*/
const POSTS_SUFFIX_PATTERN = /posts|Posts.*/
...
export class ProfileParser {
  ...
  private static extractUsernameFromUrl(url: string): string | null {
    try {
      const match = url.match(TWITTER_PROFILE_URL_PATTERN)
    ...
    try {
      ...
      for (const statItem of statItems) {
        ...
        if (text.includes('Following')) {
          const countText = text.replace(FOLLOWING_SUFFIX_PATTERN, '').trim()
          stats.following = this.parseStatNumber(countText)
        }
        else if (text.includes('Followers')) {
          const countText = text.replace(FOLLOWERS_SUFFIX_PATTERN, '').trim()
          stats.followers = this.parseStatNumber(countText)
        }
        else if (text.includes('posts') || text.includes('Posts')) {
          const countText = text.replace(POSTS_SUFFIX_PATTERN, '').trim()
