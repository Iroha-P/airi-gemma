...
const THINK_BLOCK_PATTERN = /<think>[\s\S]*?<\/think>/
const LEADING_JSON_FENCE_PATTERN = /^```json\s*\n/
...
export function parseMayStructuredMessage(responseText: string) {
  ...
  if (candidate.startsWith('{') && candidate.endsWith('}')) {
    logger.withField('text', JSON.stringify(responseText)).withField('result', candidate).log('Multiple messages detected')

    const parsedResponse = parse(candidate) as ({ messages?: unknown, reply_to_message_id?: unknown } | undefined)
...
export async function sendMessage(
  ...
  res.text = res.text
    ...
    .replace(LEADING_JSON_FENCE_PATTERN, '')
