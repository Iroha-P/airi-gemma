...
const FRAME_INDEX_PATTERN = /frame-(\d+)\.png/
    ...
    ffmpeg(inputFilePath)
      ...
      .on('end', async () => {
        ...
        const sortedFiles = files
          .filter(file => file.match(FRAME_FILE_PATTERN))
          .sort((a, b) => {
            const numA = Number.parseInt(a.match(FRAME_INDEX_PATTERN)[1])
            const numB = Number.parseInt(b.match(FRAME_INDEX_PATTERN)[1])
            return numA - numB
          })
  ...
  try {
    ...
    for (const frame of frames) {
      try {
        ...
        res.text = res.text.replace(THINK_BLOCK_PATTERN, '').trim()
        if (!res.text) {
