...
import { registerGlobalAgentService, setupAgentOrchestrator } from './services/airi/agent'
import { createWindowAuthManagerService } from './services/airi/auth'
import { setupServerChannel } from './services/airi/channel-server'
import { createAgentChatRuntimeConfig, registerGlobalAgentChatRuntimeConfigService, resolveAgentChatRuntimeFromConfig } from './services/airi/chat-runtime/config'
import { registerGlobalComputerUseService, setupComputerUseManager } from './services/airi/computer-use'
import { registerGlobalDreamService, setupDreamManager, setupDreamScheduler } from './services/airi/dream'
import { setupMcpStdioManager } from './services/airi/mcp-servers'
import { registerGlobalMemoryService, setupMemoryManager } from './services/airi/memory'
import { setupPluginHost } from './services/airi/plugins'
import { registerGlobalRoutineService, setupRoutineManager } from './services/airi/routines'
...
app.whenReady().then(async () => {
  ...
  const memoryManager = injeca.provide('modules:memory-manager', {
    build: async () => setupMemoryManager(),
  })

  const routineManager = injeca.provide('modules:routine-manager', {
    build: async () => setupRoutineManager(),
  })

  const computerUseManager = injeca.provide('modules:computer-use-manager', {
    build: async () => setupComputerUseManager(),
  })

  const agentChatRuntimeConfig = injeca.provide('configs:agent-chat-runtime', {
    build: () => createAgentChatRuntimeConfig(),
  })

  const agentOrchestrator = injeca.provide('modules:agent-orchestrator', {
    dependsOn: { memoryManager, routineManager, computerUseManager, agentChatRuntimeConfig },
    build: ({ dependsOn }) => setupAgentOrchestrator({
      memoryManager: dependsOn.memoryManager,
      chatRuntimeConfig: dependsOn.agentChatRuntimeConfig.get(),
      onChatRuntimeConfigError: error => log.withError(error).warn('failed to enable configured agent chat runtime; falling back to deterministic responses'),
      routineManager: dependsOn.routineManager,
      computerUseManager: dependsOn.computerUseManager,
    }),
  })

  const dreamManager = injeca.provide('modules:dream-manager', {
    dependsOn: { memoryManager, agentChatRuntimeConfig },
    build: ({ dependsOn }) => setupDreamManager({
      agentChatRuntimeConfig: dependsOn.agentChatRuntimeConfig,
      memoryManager: dependsOn.memoryManager,
    }),
  })

  const dreamScheduler = injeca.provide('modules:dream-scheduler', {
    dependsOn: { dreamManager },
    build: ({ dependsOn }) => setupDreamScheduler({
      manager: dependsOn.dreamManager,
    }),
  })

  const memoryRpc = injeca.provide('services:memory-rpc', {
    dependsOn: { memoryManager },
    build: ({ dependsOn }) => registerGlobalMemoryService({ manager: dependsOn.memoryManager }),
  })

  const dreamRpc = injeca.provide('services:dream-rpc', {
    dependsOn: { dreamManager, dreamScheduler },
    build: ({ dependsOn }) => registerGlobalDreamService({
      manager: dependsOn.dreamManager,
      scheduler: dependsOn.dreamScheduler,
    }),
  })

  const routineRpc = injeca.provide('services:routine-rpc', {
    dependsOn: { routineManager },
    build: ({ dependsOn }) => registerGlobalRoutineService({ manager: dependsOn.routineManager }),
  })

  const computerUseRpc = injeca.provide('services:computer-use-rpc', {
    dependsOn: { computerUseManager },
    build: ({ dependsOn }) => registerGlobalComputerUseService({ manager: dependsOn.computerUseManager }),
  })

  const agentRpc = injeca.provide('services:agent-rpc', {
    dependsOn: { agentOrchestrator },
    build: ({ dependsOn }) => registerGlobalAgentService({ orchestrator: dependsOn.agentOrchestrator }),
  })

  const agentChatRuntimeConfigRpc = injeca.provide('services:agent-chat-runtime-config-rpc', {
    dependsOn: { agentChatRuntimeConfig, agentOrchestrator },
    build: ({ dependsOn }) => registerGlobalAgentChatRuntimeConfigService({
      config: dependsOn.agentChatRuntimeConfig,
      onConfigApplied: (config) => {
        dependsOn.agentOrchestrator.configureChatRuntime(resolveAgentChatRuntimeFromConfig(config))
      },
    }),
  })
  ...
  injeca.invoke({
    dependsOn: { mainWindow, tray, serverChannel, pluginHost, mcpStdioManager, memoryManager, routineManager, computerUseManager, agentOrchestrator, dreamManager, dreamScheduler, memoryRpc, dreamRpc, routineRpc, computerUseRpc, agentRpc, agentChatRuntimeConfigRpc, onboardingWindow: onboardingWindowManager },
    callback: noop,
  })
