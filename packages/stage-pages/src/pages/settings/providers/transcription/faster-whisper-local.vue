...
import { FieldCombobox, FieldSelect } from '@proj-airi/ui'
import { storeToRefs } from 'pinia'
import { computed, onMounted, ref, watch } from 'vue'
import { useI18n } from 'vue-i18n'

const providerId = 'faster-whisper-local'
...
const model = computed({
  get: () => providers.value[providerId]?.model || 'small',
  set: (value) => {
    if (!providers.value[providerId])
      providers.value[providerId] = {}
...
async function handleGenerateTranscription(file: File) {
  ...
  const modelToUse = model.value || 'small'
      ...
      <ProviderSettingsContainer class="w-full md:w-[40%]">
        ...
        >
          ...
          <div class="flex items-center gap-2 rounded-lg bg-neutral-100 p-3 dark:bg-neutral-800">
            ...
            />
            <span class="text-sm">
              {{ serverStatus === 'online' ? t('settings.pages.providers.provider.faster-whisper-local.fields.field.status.online') : serverStatus === 'offline' ? t('settings.pages.providers.provider.faster-whisper-local.fields.field.status.offline') : t('settings.pages.providers.provider.faster-whisper-local.fields.field.status.checking') }}
          ...
          <FieldCombobox
            ...
            v-model="model"
            :label="t('settings.pages.providers.provider.faster-whisper-local.fields.field.model.label')"
            :description="t('settings.pages.providers.provider.faster-whisper-local.fields.field.model.description')"
          ...
          <FieldSelect
            v-model="device"
            :label="t('settings.pages.providers.provider.faster-whisper-local.fields.field.device.label')"
            :description="t('settings.pages.providers.provider.faster-whisper-local.fields.field.device.description')"
            :options="[
              { label: 'CPU', value: 'cpu' },
