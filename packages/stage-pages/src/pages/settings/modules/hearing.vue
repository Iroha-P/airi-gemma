...
// Local STT server device (CPU/GPU) management
const LOCAL_STT_PROVIDERS = ['faster-whisper-local', 'paraformer-local'] as const
const isLocalSttProvider = computed(() => LOCAL_STT_PROVIDERS.includes(activeTranscriptionProvider.value as any))
const localSttServerStatus = ref<'unknown' | 'online' | 'offline'>('unknown')
const localSttServerDevice = ref('')
const isSwitchingDevice = ref(false)
const localSttDevice = computed({
  get: () => {
    if (!isLocalSttProvider.value)
      return 'cpu'
    const config = providersStore.getProviderConfig(activeTranscriptionProvider.value)
    return (config.device as string) || 'cpu'
  },
  set: (value: string) => {
    if (!isLocalSttProvider.value)
      return
...
async function checkLocalSttServer() {
  if (!isLocalSttProvider.value)
    return
  const config = providersStore.getProviderConfig(activeTranscriptionProvider.value)
  const baseUrl = ((config.baseUrl as string) || 'http://127.0.0.1:8000/v1/').replace(OPENAI_V1_SUFFIX_PATTERN, '').replace(TRAILING_SLASH_PATTERN, '')
  try {
    const resp = await globalThis.fetch(baseUrl, { signal: AbortSignal.timeout(3000) })
    if (resp.ok) {
      const data = await resp.json()
      localSttServerStatus.value = 'online'
      localSttServerDevice.value = data.device || 'unknown'
    }
    else { localSttServerStatus.value = 'offline' }
  }
  catch { localSttServerStatus.value = 'offline' }
}

async function switchLocalSttDevice(newDevice: string) {
  if (!isLocalSttProvider.value || newDevice === localSttServerDevice.value)
    return
  ...
  try {
    ...
    const resp = await globalThis.fetch(`${baseUrl}/config`, { method: 'POST', body: formData })
    if (resp.ok) {
      localSttDevice.value = newDevice
      await checkLocalSttServer()
    }
  }
  catch (err) { console.error('Failed to switch STT device:', err) }
  finally { isSwitchingDevice.value = false }
}

watch(activeTranscriptionProvider, async () => {
  if (isLocalSttProvider.value)
    await checkLocalSttServer()
})

watch(localSttDevice, async (newDevice) => {
  if (isLocalSttProvider.value && localSttServerStatus.value === 'online' && newDevice !== localSttServerDevice.value)
    await switchLocalSttDevice(newDevice)
})
...
onMounted(async () => {
  ...
  syncOpenAICompatibleSettings()
  // Check local STT server status if applicable
  if (isLocalSttProvider.value)
    await checkLocalSttServer()
})
  ...
  <div flex="~ col md:row gap-6">
    <div bg="neutral-100 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-4" class="h-fit w-full md:w-[40%]">
      <div flex="~ col gap-4">
        ...
        <!-- Local STT Server Device Settings (CPU/GPU) -->
        <div v-if="isLocalSttProvider" class="border-t border-neutral-200 pt-4 dark:border-neutral-700">
          <div class="mb-4">
            <h2 class="text-lg text-neutral-500 md:text-2xl dark:text-neutral-500">
              {{ t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.device.label`) }}
            </h2>
          </div>

          <!-- Server Status -->
          <div class="mb-4 flex items-center gap-2 rounded-lg bg-neutral-100 p-3 dark:bg-neutral-800">
            <div
              class="h-2.5 w-2.5 rounded-full"
              :class="{
                'bg-green-500': localSttServerStatus === 'online',
                'bg-red-500': localSttServerStatus === 'offline',
                'bg-yellow-500': localSttServerStatus === 'unknown',
              }"
            />
            <span class="text-sm">
              {{ localSttServerStatus === 'online'
                ? t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.status.online`)
                : localSttServerStatus === 'offline'
                  ? t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.status.offline`)
                  : t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.status.checking`) }}
              <span v-if="localSttServerStatus === 'online' && localSttServerDevice" class="text-neutral-500">
                ({{ localSttServerDevice.toUpperCase() }})
              </span>
            </span>
          </div>

          <FieldSelect
            v-model="localSttDevice"
            :label="t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.device.label`)"
            :description="t(`settings.pages.providers.provider.${activeTranscriptionProvider}.fields.field.device.description`)"
            :options="[
              { label: 'CPU', value: 'cpu' },
              { label: 'CUDA (GPU)', value: 'cuda' },
            ]"
            :disabled="isSwitchingDevice || localSttServerStatus !== 'online'"
          />
        </div>
