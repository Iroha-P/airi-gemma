...
const ACT_EMOTION_PATTERN = /<\|ACT\s*(?::\s*)?(\{[\s\S]*\})\|>/i
const DELAY_PATTERN = /<\|DELAY:\d+\|>/i
const DELAY_CAPTURE_PATTERN = /<\|DELAY:(\d+)\|>/i
...
export function useEmotionsMessageQueue(emotionsQueue: UseQueueReturn<EmotionPayload>) {
  ...
  function parseActEmotion(content: string) {
    const match = ACT_EMOTION_PATTERN.exec(content)
    if (!match)
...
export function useDelayMessageQueue() {
  function splitDelays(content: string) {
    ...
    const delayExecArray = DELAY_CAPTURE_PATTERN.exec(content)
