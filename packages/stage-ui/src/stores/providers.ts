...
export const useProvidersStore = defineStore('providers', () => {
  ...
  const providerMetadata: Record<string, ProviderMetadata> = {
    ...
    'faster-whisper-local': {
      ...
      category: 'transcription',
      tasks: ['speech-to-text', 'automatic-speech-recognition', 'asr', 'stt'],
      nameKey: 'settings.pages.providers.provider.faster-whisper-local.title',
      name: 'Faster Whisper (Local)',
      descriptionKey: 'settings.pages.providers.provider.faster-whisper-local.description',
      description: 'Local faster-whisper STT server. Supports CPU/GPU switching.',
      ...
      createProvider: async (config: Record<string, unknown>) => {
        const apiKey = typeof config.apiKey === 'string' ? config.apiKey.trim() : 'sk-local'
        let baseUrl = typeof config.baseUrl === 'string' ? config.baseUrl.trim() : 'http://127.0.0.1:8000/v1/'
      ...
      capabilities: {
        listModels: async () => [
          { id: 'small', name: 'Whisper Small (~1GB)', provider: 'faster-whisper-local', description: 'Fast, good accuracy for most languages', contextLength: 0, deprecated: false },
          { id: 'medium', name: 'Whisper Medium (~2GB)', provider: 'faster-whisper-local', description: 'Better accuracy, slower', contextLength: 0, deprecated: false },
          { id: 'large-v3', name: 'Whisper Large V3 (~3GB)', provider: 'faster-whisper-local', description: 'Best accuracy, requires more VRAM', contextLength: 0, deprecated: false },
          { id: 'tiny', name: 'Whisper Tiny (~150MB)', provider: 'faster-whisper-local', description: 'Fastest, lowest accuracy', contextLength: 0, deprecated: false },
          { id: 'base', name: 'Whisper Base (~300MB)', provider: 'faster-whisper-local', description: 'Fast with decent accuracy', contextLength: 0, deprecated: false },
        ],
      },
      validators: {
        chatPingCheckAvailable: false,
        validateProviderConfig: async (config: Record<string, unknown>) => {
          ...
          if (baseUrl && errors.length === 0) {
            try {
              const url = baseUrl.replace(OPENAI_V1_SUFFIX_PATTERN, '').replace(TRAILING_SLASH_PATTERN, '')
              const resp = await globalThis.fetch(url, { signal: AbortSignal.timeout(5000) })
              if (!resp.ok)
                errors.push(new Error(`Whisper server returned HTTP ${resp.status}`))
            }
            catch (err) {
              errors.push(new Error(`Cannot reach Whisper server at ${baseUrl}: ${String(err)}`))
            }
          }
          return {
            errors,
    ...
    'paraformer-local': {
      ...
      category: 'transcription',
      tasks: ['speech-to-text', 'automatic-speech-recognition', 'asr', 'stt'],
      nameKey: 'settings.pages.providers.provider.paraformer-local.title',
      name: 'Paraformer (Local)',
      descriptionKey: 'settings.pages.providers.provider.paraformer-local.description',
      description: 'Local Paraformer (FunASR) STT server. Optimized for Chinese. Supports CPU/GPU switching.',
      icon: 'i-material-symbols:mic',
      defaultOptions: () => ({
        baseUrl: 'http://127.0.0.1:8000/v1/',
        apiKey: 'sk-local',
      ...
      capabilities: {
        listModels: async () => [
          { id: 'paraformer-zh', name: 'Paraformer Chinese', provider: 'paraformer-local', description: 'Chinese speech recognition with VAD and punctuation', contextLength: 0, deprecated: false },
        ],
      },
      validators: {
        chatPingCheckAvailable: false,
        validateProviderConfig: async (config: Record<string, unknown>) => {
          ...
          if (baseUrl && errors.length === 0) {
            try {
              ...
              const resp = await globalThis.fetch(url, { signal: AbortSignal.timeout(5000) })
              if (!resp.ok)
                errors.push(new Error(`Paraformer server returned HTTP ${resp.status}`))
            }
            catch (err) {
              errors.push(new Error(`Cannot reach Paraformer server at ${baseUrl}: ${String(err)}`))
            }
          }
          return {
            errors,
    ...
    'gpt-sovits-tts': {
      ...
      category: 'speech',
      tasks: ['text-to-speech', 'tts'],
      nameKey: 'settings.pages.providers.provider.gpt-sovits-tts.title',
      name: 'GPT-SoVITS',
      descriptionKey: 'settings.pages.providers.provider.gpt-sovits-tts.description',
      description: 'Local GPT-SoVITS text-to-speech server with voice cloning.',
      icon: 'i-material-symbols:record-voice-over',
      ...
      createProvider: async (config) => {
        const provider: SpeechProvider = {
          speech: () => ({
            baseURL: (config.baseUrl as string) || 'http://127.0.0.1:9881',
            model: 'gpt-sovits',
            fetch: async (_input: RequestInfo | URL, init?: RequestInit) => {
              ...
              const baseUrl = ((config.baseUrl as string) || 'http://127.0.0.1:9881').replace(TRAILING_SLASH_PATTERN, '')
              const textLang = (config.textLang as string) || 'ja'
              const streaming = config.streamingMode !== false

              const refPath = (config.refAudioPath as string) || ''
              if (!refPath)
                throw new Error('Reference audio path is not configured. Please set it in GPT-SoVITS settings.')
              // Ensure absolute path (Windows: drive letter like C:\, D:\, etc.)
              if (!WINDOWS_ABSOLUTE_PATH_PATTERN.test(refPath) && !refPath.startsWith('/'))
                throw new Error(`Reference audio path must be an absolute path (e.g. F:\\gsv\\ref.wav), got: ${refPath}`)

              const params = new URLSearchParams({
                text,
                text_lang: textLang,
                ref_audio_path: refPath,
                prompt_text: (config.promptText as string) || '',
                prompt_lang: (config.promptLang as string) || 'ja',
                media_type: 'wav',
                streaming_mode: streaming ? 'true' : 'false',
                top_k: String(config.topK ?? 12),
                top_p: String(config.topP ?? 0.8),
                temperature: String(config.temperature ?? 0.8),
                speed: String(body.speed ?? config.speed ?? 1.0),
                text_split_method: textLang === 'zh' ? 'cut5' : ((config.textSplitMethod as string) || 'cut0'),
                batch_size: '1',
                repetition_penalty: String(config.repetitionPenalty ?? 1.35),
              })

              const response = await globalThis.fetch(`${baseUrl}/tts?${params.toString()}`)
              if (!response.ok) {
                const errorText = await response.text()
                throw new Error(`GPT-SoVITS API error (HTTP ${response.status}): ${errorText}`)
              }

              if (streaming && response.body) {
                // Stream mode: pipe the chunked audio response through
      ...
      validators: {
        ...
        validateProviderConfig: async (config: Record<string, unknown>) => {
          ...
          // Health check: try to reach the GPT-SoVITS server (root returns 404, so just check connectivity)
          if (baseUrl && errors.length === 0) {
            try {
              const url = baseUrl.replace(TRAILING_SLASH_PATTERN, '')
              const resp = await globalThis.fetch(url, { signal: AbortSignal.timeout(5000) })
              // GPT-SoVITS root path returns 404 but that means the server IS running
              if (!resp.ok && resp.status !== 404)
                errors.push(new Error(`GPT-SoVITS server returned HTTP ${resp.status}`))
            }
            catch (err) {
              errors.push(new Error(`Cannot reach GPT-SoVITS server at ${baseUrl}: ${String(err)}`))
            }
          }
